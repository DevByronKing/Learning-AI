    ...
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
      ...
      <div className="mt-8 grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
        ...
        <div className="glass-panel p-5 rounded-3xl border border-indigo-500/30 relative overflow-hidden glow-brand">
          ...
          <div className="mt-3 flex items-baseline gap-2">
            <span className="text-3xl font-black text-slate-900 dark:text-white">{metrics.probabilityOfPassing}%</span>
            <span className="text-xs font-bold text-emerald-400">+4.2% esta semana</span>
          </div>
          <p className="text-[11px] text-slate-500 dark:text-slate-400 mt-1">Nota de corte estimada: {metrics.estimatedCutoffScore}%</p>
        ...
        <div className="glass-panel p-5 rounded-3xl border border-slate-300 dark:border-white/10">
          ...
          <div className="mt-3 flex items-baseline gap-2">
            ...
            <span className="text-xs text-slate-500 dark:text-slate-400">Recorde: 22d</span>
          </div>
          <p className="text-[11px] text-slate-500 dark:text-slate-400 mt-1">Consolidação contínua de memória</p>
        </div>
        ...
        <div className="glass-panel p-5 rounded-3xl border border-slate-300 dark:border-white/10">
          ...
          <div className="mt-3 flex items-baseline gap-2">
            <span className="text-3xl font-black text-slate-900 dark:text-white">{metrics.totalAnswered}</span>
            <span className="text-xs text-emerald-400 font-bold">{metrics.totalCorrect} acertos</span>
          </div>
          <p className="text-[11px] text-slate-500 dark:text-slate-400 mt-1">Tempo médio por questão: 1m 42s</p>
        </div>
      ...
      <div className="mt-10 glass-panel p-6 sm:p-8 rounded-3xl border border-slate-300 dark:border-white/10">
        ...
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 pb-6 border-b border-slate-300 dark:border-white/10">
          ...
          <div className="flex flex-wrap items-center gap-3 text-xs">
            <span className="flex items-center gap-1.5">
              ...
              <span className="text-slate-600 dark:text-slate-300">Ponto Crítico (&lt; 50%)</span>
            </span>
            <span className="flex items-center gap-1.5">
              <span className="w-3 h-3 rounded-full bg-amber-500" />
              <span className="text-slate-600 dark:text-slate-300">Em Desenvolvimento (50-70%)</span>
            </span>
            <span className="flex items-center gap-1.5">
              <span className="w-3 h-3 rounded-full bg-emerald-500" />
              <span className="text-slate-600 dark:text-slate-300">Consolidado (&gt; 70%)</span>
            </span>
          </div>
        </div>
        ...
        <div className="mt-8 space-y-6">
          {selectedExam.subjects.map((subject) => (
            <div key={subject.id} className="bg-white dark:bg-dark-surface/60 p-5 rounded-2xl border border-slate-200 dark:border-white/5">
                    ...
                    >
                      <div>
                        ...
                        {topic.articlesOrLaws && topic.articlesOrLaws.length > 0 && (
                          <p className="text-[10px] text-slate-500 dark:text-slate-400 font-mono mt-2 truncate">
                            📜 {topic.articlesOrLaws[0]}
                          </p>
                        )}
                      </div>

                      <div className="mt-3 pt-2 border-t border-slate-300 dark:border-white/10 flex items-center justify-between text-[10px]">
                        <span className="font-semibold uppercase tracking-wider">{topic.status}</span>
                        <span className="text-slate-600 dark:text-slate-300">Cobrança {topic.frequencyInBanca}</span>
                      </div>
                    </div>
                  );
                })}
      ...
      <div className="mt-10 grid grid-cols-1 lg:grid-cols-2 gap-8">
        ...
        <div className="glass-panel p-6 sm:p-8 rounded-3xl border border-slate-300 dark:border-white/10 flex flex-col justify-between">
          ...
          <div className="grid grid-cols-2 gap-2 mt-4 pt-4 border-t border-slate-200 dark:border-white/5 text-xs">
            {errorData.map((e) => (
              <div key={e.name} className="flex items-center gap-2 text-slate-600 dark:text-slate-300">
        ...
        <div className="mt-10 glass-panel p-6 sm:p-8 rounded-3xl border border-indigo-500/30 glow-brand">
          ...
          >
            <div>
              ...
              <p className="text-sm sm:text-base text-slate-900 dark:text-white font-medium leading-relaxed whitespace-pre-line">
                {isFlipped ? currentFlashcard.back : currentFlashcard.front}
              </p>
            </div>

            <div className="mt-6 pt-3 border-t border-slate-200 dark:border-white/5 flex items-center justify-between text-xs text-slate-500 dark:text-slate-400">
