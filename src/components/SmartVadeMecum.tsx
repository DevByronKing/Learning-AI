  ...
  return (
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8 space-y-8 animate-fadeIn">
      ...
      <div className="relative rounded-3xl bg-gradient-to-r from-amber-950/30 via-dark-surface to-indigo-950/30 border border-amber-500/20 p-6 sm:p-8 backdrop-blur-xl shadow-2xl overflow-hidden">
        ...
        <div className="flex flex-wrap items-center gap-4 mt-6 pt-4 border-t border-white/5 text-xs text-slate-400">
          <span className="font-bold text-slate-300">Legenda de Grifos:</span>
          ...
          <button
            ...
            className={`px-4 py-2 rounded-xl text-xs font-bold whitespace-nowrap transition-all ${
              selectedDiploma === d.id
                ...
                : 'bg-dark-surface/80 border border-white/5 text-slate-400 hover:text-white hover:bg-white/5'
            }`}
          >
            {d.name}
          </button>
      ...
      {/* Search & Filters */}
      <div className="flex flex-col sm:flex-row items-stretch sm:items-center justify-between gap-4 bg-dark-surface/70 border border-white/5 p-4 rounded-2xl backdrop-blur-xl">
        <div className="relative flex-1">
          ...
          <input
            ...
            className="w-full bg-dark-bg border border-white/10 rounded-xl pl-10 pr-4 py-2 text-xs text-slate-200 focus:outline-none focus:border-amber-500"
          />
        </div>

        <div className="flex items-center gap-3">
          <span className="text-xs text-slate-400 font-medium shrink-0">Incidência:</span>
          <select
            value={selectedIncidence}
            onChange={(e) => setSelectedIncidence(e.target.value)}
            className="bg-dark-bg border border-white/10 rounded-xl px-3 py-2 text-xs text-slate-200 focus:outline-none focus:border-amber-500"
          >
      ...
      <div className="space-y-6">
        {filteredArticles.length === 0 ? (
          <div className="text-center py-16 bg-dark-surface/40 border border-white/5 rounded-3xl space-y-3">
            <AlertCircle className="w-10 h-10 text-slate-500 mx-auto" />
            <h3 className="text-base font-bold text-white">Nenhum artigo encontrado</h3>
            <p className="text-xs text-slate-400">Tente buscar por outro termo ou selecione todos os diplomas.</p>
          </div>
        ) : (
          filteredArticles.map((art) => (
            <div
              key={art.id}
              className="rounded-2xl bg-dark-surface/90 border border-white/5 hover:border-amber-500/30 p-6 space-y-4 transition-all shadow-lg"
            >
              {/* Article Header */}
              <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 border-b border-white/5 pb-4">
                ...
                <div className="flex items-center gap-2">
                  ...
                  <button
                    ...
                    className="p-1.5 rounded-lg bg-dark-bg hover:bg-white/5 border border-white/10 text-slate-400 hover:text-white transition-all text-xs"
              ...
              <div className="bg-dark-bg/80 border border-white/5 rounded-xl p-4 sm:p-5">
