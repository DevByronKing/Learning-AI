...
export const StudentProfileModal: React.FC<StudentProfileModalProps> = ({
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-3 sm:p-4 bg-black/80 backdrop-blur-md animate-fade-in overflow-y-auto">
      <div 
        className="relative w-full max-w-3xl my-8 rounded-3xl bg-slate-900 border border-slate-700/80 dark:border-white/10 shadow-2xl overflow-hidden flex flex-col max-h-[92vh]"
      ...
      >
        ...
        <div className="flex items-center justify-between px-6 pt-4 pb-2 border-b border-slate-800 bg-slate-900/90 shrink-0">
        ...
        <form onSubmit={handleSave} className="p-6 overflow-y-auto space-y-6 flex-1 text-slate-200">
            ...
            <div className="space-y-5">
                  ...
                  return (
                    <div
                      ...
                      className={`p-4 rounded-2xl border cursor-pointer transition-all relative flex flex-col justify-between ${
                        isSelected
                          ? 'bg-indigo-950/40 border-indigo-400 ring-2 ring-indigo-500/50 shadow-lg shadow-indigo-950/50'
                          : 'bg-slate-800/60 border-slate-700/70 hover:bg-slate-800 hover:border-slate-600'
                      }`}
                    >
                      {isSelected && (
                        <span className="absolute top-2.5 right-2.5 w-5 h-5 rounded-full bg-indigo-500 text-white flex items-center justify-center text-[10px] font-black shadow">
                          ✓
                        </span>
                      )}
                      ...
                      <div>
                        <div className="flex items-center gap-2.5">
                          <span className="text-3xl p-1.5 rounded-xl bg-slate-900/80 border border-white/5">
                            {animal.emoji}
                          </span>
                          <div className="min-w-0">
                            <h4 className="text-xs font-black text-white truncate">{animal.name}</h4>
                            <span className="text-[10px] text-indigo-400 font-bold block truncate">{animal.archetype}</span>
                          </div>
                        </div>

                        <p className="text-[11px] text-slate-300 mt-2.5 leading-relaxed line-clamp-2 font-medium">
                          {animal.superpower}
                        </p>
                      </div>

                      {/* Small stats bars */}
                      <div className="mt-3 pt-2.5 border-t border-white/5 space-y-1 text-[9px] font-mono">
                        <div className="flex items-center justify-between text-slate-400">
                          <span>Foco</span>
                          <span className="text-emerald-400 font-bold">{animal.stats.foco}%</span>
                        </div>
                        <div className="w-full h-1 bg-slate-700 rounded-full overflow-hidden">
                          <div className="h-full bg-emerald-400" style={{ width: `${animal.stats.foco}%` }} />
                        </div>
                        <div className="flex items-center justify-between text-slate-400">
                          <span>Velocidade</span>
                          <span className="text-cyan-400 font-bold">{animal.stats.velocidade}%</span>
                        </div>
                        <div className="w-full h-1 bg-slate-700 rounded-full overflow-hidden">
                          <div className="h-full bg-cyan-400" style={{ width: `${animal.stats.velocidade}%` }} />
                        </div>
                      </div>
                    </div>
                  );
              ...
              <div className="p-4 rounded-2xl bg-indigo-950/20 border border-indigo-500/30 text-xs space-y-2">
                ...
                <div className="flex flex-wrap items-center gap-1.5 pt-1">
                  <span className="text-[10px] text-slate-400 font-bold">Carreiras Recomendadas:</span>
                  {selectedAnimal.bestForCareers.map((car, idx) => (
                    <span key={idx} className="px-2 py-0.5 rounded-md bg-indigo-500/20 text-indigo-200 text-[10px] font-medium border border-indigo-500/20">
                      {car}
                    </span>
                  ))}
                </div>
          ...
          {activeSubTab === 'dados' && (
            <div className="space-y-5">
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                <div>
                  ...
                  <input
                    ...
                    className="w-full px-4 py-2.5 rounded-xl bg-slate-800 border border-slate-700 text-white text-xs font-medium focus:outline-none focus:border-indigo-500 transition-colors"
                  />
                </div>
              ...
              <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
                <div>
                  ...
                  <select
                    ...
                    className="w-full px-3 py-2.5 rounded-xl bg-slate-800 border border-slate-700 text-white text-xs font-medium focus:outline-none focus:border-indigo-500 transition-colors"
                  >
              ...
              <div className="p-4 rounded-2xl bg-slate-800/80 border border-slate-700 flex flex-col sm:flex-row sm:items-center justify-between gap-3 text-xs">
                <div>
                  <span className="text-[10px] uppercase font-bold text-slate-400">Seu Status Atual</span>
